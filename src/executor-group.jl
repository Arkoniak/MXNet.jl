...
type DataParallelExecutorGroup <: AbstractExecutorGroup
  ...
  for_training :: Bool
  freeze_param_names :: Union{Void, Vector{Symbol}}
  ...
  DataParallelExecutorGroup(arch :: SymbolicNode, context, for_training, freeze_param_names) = new(arch, context, for_training, freeze_param_names)
end
function DataParallelExecutorGroup(arch :: SymbolicNode, context :: Vector{Context},
  ...
  bind_exec!(exec_group, data, shared_group, inputs_need_grad, grad_req)
end
...
function bind_exec!(self :: DataParallelExecutorGroup,
  ...
  batch_size  = get_batch_size(data_provider)
  slices      = _split_inputs(batch_size, num_dev)

  arg_names    = list_arguments(self.arch)
  data_names   = [x[1] for x in provide_data(data_provider)]
  label_names  = [x[1] for x in provide_label(data_provider)]
  ...
  if isa(self.freeze_param_names, Void)
  ...
  for i = 1:num_dev
    data_shapes = Dict([k => tuple(v[1:end-1]...,length(slices[i])) for (k,v) in provide_data(data_provider)])
    label_shapes = Dict([k => tuple(v[1:end-1]...,length(slices[i])) for (k,v) in provide_label(data_provider)])
