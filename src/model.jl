...
function fit(self :: FeedForward, optimizer :: AbstractOptimizer, data :: AbstractDataProvider; kwargs...)
  ...
  for i = 1:num_dev
    data_shapes = map((x) -> x[1] => tuple(x[2][1:end-1]...,length(slices[i])), provide_data(data))
    label_shapes = map((x) -> x[1] => tuple(x[2][1:end-1]...,length(slices[i])), provide_label(data))
...
function save_checkpoint(sym :: SymbolicNode, arg_params :: Dict{Base.Symbol, NDArray},
  ...
  save("$prefix-symbol.json", sym)
  save_dict = merge(map((x) -> Symbol("arg:$(x[1])") => x[2], arg_params),
                    map((x) -> Symbol("aux:$(x[1])") => x[2], aux_params))
